...
export default function AISidebarChat() {
  ...
  if (isLoading) {
    return (
      <div
        className="absolute right-0 top-0 bottom-0 h-full glass border-l border-border/80 flex flex-col z-20 shadow-2xl shadow-black/50"
        style={{ width }}
      >
        <div
        ...
        <div className="border-b border-border/60">
          <div className="flex items-center justify-between px-4 pt-4 pb-3">
            <div className="flex items-center space-x-3">
              <div className="w-9 h-9 bg-card rounded-lg animate-pulse" />
              <div>
                <div className="w-24 h-4 bg-card rounded animate-pulse mb-1" />
            ...
            {[1, 2, 3].map((i) => (
              <div key={i} className={`flex ${i % 2 === 0 ? "justify-end" : "justify-start"}`}>
                <div className={`space-y-2 ${i % 2 === 0 ? "max-w-[85%]" : "max-w-[90%]"}`}>
                  <div className={`h-8 bg-card rounded-2xl animate-pulse ${i % 2 === 0 ? "rounded-br-sm" : "rounded-bl-sm"}`} style={{ width: `${60 + Math.random() * 40}%` }} />
                  <div className="h-3 bg-card rounded animate-pulse ml-2" style={{ width: "40px" }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="p-4 border-t border-border/60">
          <div className="h-11 bg-card rounded-lg animate-pulse" />
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      {showConvList ? (
        ...
        <>
        <div className="flex-1 overflow-y-auto p-4 bg-background relative" ref={chatContainerRef} onScroll={handleScroll}>
          <div className="flex flex-col gap-3">
