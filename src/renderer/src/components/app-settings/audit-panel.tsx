...
export function AuditPanel() {
  ...
  return (
    <div className="space-y-3">
      <div>
        <p className="text-foreground/80 text-sm font-semibold">Audit log</p>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button
          ...
          size="sm"
          variant="outline"
          className="gap-1.5 border-border/80 bg-card/50"
        >
          <KeyRound size={13} />
          Copy public key
        </Button>
        <Button
          ...
          size="sm"
          variant="outline"
          className="gap-1.5 border-border/80 bg-card/50 hover:bg-red-500/10 hover:text-red-400"
        >
          <Trash2 size={13} />
          Clear log
        </Button>
      ...
      <div className="space-y-1 max-h-[280px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-accent-600/40 scrollbar-track-transparent">
        {entries.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            <FileSignature size={26} className="text-muted-foreground/60 mb-2" />
            <p className="text-sm text-muted-foreground/70">No audit entries yet</p>
          </div>
        )}
        {entries
          .slice()
          ...
          .map((e, i) => (
            <div
              key={`${e.ts}-${i}`}
              className="flex items-baseline gap-2 px-2 py-1 rounded bg-card/40 text-[11px]"
            >
