...
const APP_FONT_OPTIONS = [
  ...
  { value: 'System Default', label: 'System Default', fontFamily: 'system-ui, -apple-system, sans-serif' },
];
...
export default function SettingsDialog({ children }) {
  ...
  const [localSettings, setLocalSettings] = useState({
    ...
    appFont: appFont || "JetBrains Mono",
  });
  ...
  const handleSave = () => {
    ...
    setAppFont(localSettings.appFont);
    setOpen(false);
  };
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-hidden bg-slate-900 border-slate-700/40 shadow-xl flex flex-col">
          ...
          <Tabs defaultValue="ai" className="flex flex-col flex-1 min-h-0">
            ...
            {/* Appearance Tab */}
            <TabsContent value="appearance" className="flex-1 overflow-y-auto px-1 space-y-5 mt-4">
              <div className="space-y-3">
                ...
                <div className="grid gap-2.5">
                  <Label className="text-gray-300 text-sm">Application Font</Label>
                  <p className="text-xs text-gray-400">Changes the font used across the entire app interface</p>
                  <Select
                    value={localSettings.appFont}
                    onValueChange={(value) =>
                      setLocalSettings({ ...localSettings, appFont: value })
                    }
                  >
                    <SelectTrigger className="bg-slate-800/60 border-slate-700/50 text-gray-100 hover:bg-slate-800 focus:border-cyan-500/50 focus:ring-cyan-500/20">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent className="bg-slate-900 border-slate-700/50">
                      {APP_FONT_OPTIONS.map((font) => (
                        <SelectItem
                          key={font.value}
                          value={font.value}
                          className="text-gray-100 focus:bg-cyan-500/20 focus:text-cyan-100"
                        >
                  ...
                  {localSettings.appFont !== appFont && (
                    <p className="text-xs text-amber-400/80">Save changes to apply the new font</p>
                  )}
                </div>
              </div>
            </TabsContent>
