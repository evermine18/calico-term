...
export function EnvVaultPanel() {
  ...
  return (
    <div className="space-y-3">
      <div>
        <p className="text-foreground/80 text-sm font-semibold">Env Var Vault</p>
      ...
      <div className="grid gap-1.5">
        <Label className="text-foreground/80 text-sm">Scope</Label>
        <Select value={scopeId} onValueChange={setScopeId}>
          <SelectTrigger className="bg-card/50 border-border/80 text-foreground">
            <SelectValue />
          </SelectTrigger>
          <SelectContent className="bg-card border-border/80">
            {(["Global", "Workspace", "Host"] as const).map((g) => {
              ...
              return (
                <div key={g}>
                  <div className="px-2 pt-1.5 pb-0.5 text-[10px] uppercase tracking-widest text-muted-foreground/70">
                    {g}
                  </div>
                  {items.map((o) => (
      ...
      <div className="space-y-2 max-h-[260px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-accent-600/40 scrollbar-track-transparent">
        {entries.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            <Variable size={28} className="text-muted-foreground/60 mb-2" />
            <p className="text-sm text-muted-foreground/70">No variables in this scope</p>
          </div>
        )}
        {entries.map((e) => (
          <div
          ...
          >
            ...
            <Button
              ...
              variant="ghost"
              size="icon"
              className="h-7 w-7 text-muted-foreground hover:text-accent-300 hover:bg-accent-500/20"
            >
      ...
      <div className="grid grid-cols-[1fr_2fr_auto] gap-2 items-end pt-2 border-t border-border/60">
        ...
        <div className="grid gap-1">
          <Label className="text-foreground/80 text-xs">Value</Label>
          <Input
            placeholder="value"
            value={newValue}
