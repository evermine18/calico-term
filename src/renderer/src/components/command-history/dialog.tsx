...
export default function CommandHistoryDialog() {
  ...
  return (
    ...
    >
      <div
        className="bg-card/98 border border-border/80 rounded-xl shadow-2xl w-[700px] max-h-[600px] flex flex-col"
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-border/40">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-card/50 border border-border/80 rounded-lg
                       text-foreground placeholder-gray-500 focus:outline-none focus:border-accent-500/50
                       focus:bg-card transition-colors text-sm"
              autoFocus
            />
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-2">
          {filteredHistory.length === 0 ? (
            ...
            filteredHistory.map((entry) => (
              <div
                key={entry.id}
                className={`group bg-card/40 hover:bg-card/60 border rounded-lg p-3 transition-all duration-150 ${entry.pinned
                    ? 'border-accent-500/50 bg-card/50'
                    : 'border-border/40 hover:border-accent-500/30'
                  }`}
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    >
                      <Copy
                        className={`w-4 h-4 ${copiedId === entry.id ? "text-green-400" : "text-muted-foreground"
                          }`}
                      />
                    </button>
                    <button
        ...
        {/* Footer */}
        <div className="px-6 py-3 border-t border-border/40 bg-card/50">
