...
export default function CommandHistoryDialog() {
  ...
  return (
    ...
    >
      <div
        className="bg-panel/98 border border-hairline/50 rounded-xl shadow-2xl w-[700px] max-h-[600px] flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-hairline/50">
        ...
        {/* Search Bar */}
        <div className="px-6 py-4 border-b border-hairline/30">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-elevated/60 border border-hairline/50 rounded-lg
                       text-ink placeholder-ink-subtle focus:outline-none focus:border-accent-500/50
                       focus:bg-elevated transition-colors text-sm"
              autoFocus
            />
          </div>
          <div className="flex items-center justify-between mt-2">
            {commandHistory.length > 0 && (
              <button
                ...
                className="flex items-center gap-2 text-xs text-danger hover:text-danger transition-colors"
              >
                <Trash2 className="w-3 h-3" />
                Clear all history
              </button>
            )}
            <p className="text-xs text-ink-subtle ml-auto">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-2">
          {filteredHistory.length === 0 ? (
            ...
            filteredHistory.map((entry) => (
              <div
                key={entry.id}
                className={`group bg-elevated/40 hover:bg-elevated/70 border rounded-lg p-3 transition-all duration-150 ${entry.pinned
                    ? 'border-accent-500/50 bg-elevated/60'
                    : 'border-hairline/30 hover:border-accent-500/30'
                  }`}
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    >
                      {entry.pinned ? (
                        <Pin className="w-4 h-4 text-accent-400 fill-accent-400" />
                      ) : (
                        <PinOff className="w-4 h-4 text-ink-muted" />
                      )}
                    </button>
                    <button
                    ...
                    >
                      <Copy
                        className={`w-4 h-4 ${copiedId === entry.id ? "text-success" : "text-ink-muted"
                          }`}
                      />
                    </button>
                    <button
        ...
        {/* Footer */}
        <div className="px-6 py-3 border-t border-hairline/30 bg-panel/50">
