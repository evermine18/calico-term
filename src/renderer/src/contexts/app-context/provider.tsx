...
export const AppProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    window.electron.ipcRenderer
      .invoke("ai-apikey-has")
      .then((has: boolean) => setHasApiKeyState(has))
      .catch(() => setHasApiKeyState(false));
  }, []);

  const setHasApiKey = (has: boolean) => setHasApiKeyState(has);

  // --- Terminal appearance ---
  const [terminalFontFamily, setTerminalFontFamilyState] = useState(
    localStorage.getItem("terminalFontFamily") ||
      "Cascadia Code, Consolas, 'Courier New', monospace",
  );
  const setTerminalFontFamily = (v: string) => {
    localStorage.setItem("terminalFontFamily", v);
    setTerminalFontFamilyState(v);
  };

  const [terminalFontSize, setTerminalFontSizeState] = useState<number>(() => {
    const s = localStorage.getItem("terminalFontSize");
    return s ? parseFloat(s) : 14;
  });
  const setTerminalFontSize = (v: number) => {
    localStorage.setItem("terminalFontSize", v.toString());
  ...
  const setTerminalLineHeight = (v: number) => {
    localStorage.setItem("terminalLineHeight", v.toString());
  ...
  const setCursorStyle = (v: "block" | "bar" | "underline") => {
    localStorage.setItem("cursorStyle", v);
    setCursorStyleState(v);
  };

  const [cursorBlink, setCursorBlinkState] = useState<boolean>(() => {
    const s = localStorage.getItem("cursorBlink");
    return s !== null ? s === "true" : true;
  });
  const setCursorBlink = (v: boolean) => {
    localStorage.setItem("cursorBlink", v.toString());
    setCursorBlinkState(v);
  };

  const [scrollback, setScrollbackState] = useState<number>(() => {
    const s = localStorage.getItem("scrollback");
    return s ? parseInt(s, 10) : 5000;
  });
  const setScrollback = (v: number) => {
    localStorage.setItem("scrollback", v.toString());
    setScrollbackState(v);
  };

  // --- Default terminal startup ---
  const [defaultShell, setDefaultShellState] = useState(
    localStorage.getItem("defaultShell") || "",
  );
  const setDefaultShell = (v: string) => {
    localStorage.setItem("defaultShell", v);
    setDefaultShellState(v);
  };

  const [defaultCwd, setDefaultCwdState] = useState(
    localStorage.getItem("defaultCwd") || "",
  );
  const setDefaultCwd = (v: string) => {
    localStorage.setItem("defaultCwd", v);
    setDefaultCwdState(v);
  };

  // --- AI advanced settings ---
  const [aiSystemPrompt, setAiSystemPromptState] = useState(
    localStorage.getItem("aiSystemPrompt") || "",
  );
  const setAiSystemPrompt = (v: string) => {
    localStorage.setItem("aiSystemPrompt", v);
    setAiSystemPromptState(v);
  };

  const [aiTemperature, setAiTemperatureState] = useState<number>(() => {
    const s = localStorage.getItem("aiTemperature");
    return s ? parseFloat(s) : 0.7;
  });
  const setAiTemperature = (v: number) => {
    localStorage.setItem("aiTemperature", v.toString());
    setAiTemperatureState(v);
  };

  const [aiMaxTokens, setAiMaxTokensState] = useState<number>(() => {
    const s = localStorage.getItem("aiMaxTokens");
    return s ? parseInt(s, 10) : 0;
  });
  const setAiMaxTokens = (v: number) => {
    localStorage.setItem("aiMaxTokens", v.toString());
    setAiMaxTokensState(v);
  };
  ...
  const setShortcuts = (v: AppShortcuts) => {
    localStorage.setItem("shortcuts", JSON.stringify(v));
    setShortcutsState(v);
  };
  ...
  const updateVaultCredential = (cred: VaultCredential) => {
    setVaultCredentials((prev) =>
      prev.map((c) => (c.id === cred.id ? cred : c)),
    );
  };
  ...
  const [commandHistory, setCommandHistory] = useState<CommandHistoryEntry[]>(
    () => {
      ...
      const retentionDays = parseFloat(
        localStorage.getItem("historyRetentionDays") || "1",
      );
